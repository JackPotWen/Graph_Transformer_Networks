...
def get_default_net_params(device, in_dim, n_classes):
    ...
    net_params = {
        ...
        'n_classes': n_classes,
        # PE融合模式控制
        'pe_fusion_mode': 'add',  # 可选: 'add' 或 'concat'
    }
    return net_params

def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='异构图Transformer训练 - 支持多个元路径PE')
    ...
    parser.add_argument('--out_dir', type=str, default='/home/kuei-jan/github/Graph_Transformer_Networks/all_attempt/PE_HeGraph_With_GT/Weighted_MTRWPE/results', help='输出目录')
    parser.add_argument('--epochs', type=int, default=50, help='训练轮数')
    parser.add_argument('--hidden_dim', type=int, default=128, help='隐藏层维度')
    parser.add_argument('--n_heads', type=int, default=8, help='注意力头数')
    parser.add_argument('--lr', type=float, default=0.001, help='学习率')
    parser.add_argument('--seed', type=int, default=42, help='随机种子')
    parser.add_argument('--pe_fusion_mode', type=str, default='add', choices=['add', 'concat'], help='PE融合模式')
    ...
    print("=" * 60)
    print("异构图Transformer训练 - 支持多个元路径PE")
    print("=" * 60)
    ...
    save_dir = os.path.join(args.out_dir, f'experiment_multi_pe_{timestamp}')
