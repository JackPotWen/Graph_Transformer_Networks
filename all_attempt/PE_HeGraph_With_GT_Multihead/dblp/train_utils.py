...
from HGT import HeterogeneousGraphTransformerNet, MetapathGraphTransformerNet, accuracy
...
def train_epoch(model, optimizer, device, data, train_mask, val_mask, test_mask):
    ...
    # 复制PE信息到训练数据
    batch_metapath2vec_pos_enc = None  # 初始化变量
    if hasattr(data, 'metapath2vec_pos_enc_list'):
        # 处理多个独立的PE
        train_data.metapath2vec_pos_enc_list = []
        for pe in data.metapath2vec_pos_enc_list:
            train_pe = pe[train_nodes].to(device)
            train_data.metapath2vec_pos_enc_list.append(train_pe)
    elif hasattr(data, 'metapath2vec_pos_enc') and data.metapath2vec_pos_enc is not None:
        # 处理单个PE（向后兼容）
        batch_metapath2vec_pos_enc = data.metapath2vec_pos_enc[train_nodes].to(device)
    
    # 复制其他PE信息
    if hasattr(data, 'metapath_names'):
        train_data.metapath_names = data.metapath_names
    if hasattr(data, 'metapath2vec_pe_paths'):
        train_data.metapath2vec_pe_paths = data.metapath2vec_pe_paths
...
def evaluate_network(model, device, data, mask, epoch):
    ...
    with torch.no_grad():
        ...
        if hasattr(data, 'metapath2vec_pos_enc_list'):
            # 处理多个独立的PE
            eval_data.metapath2vec_pos_enc_list = []
            for pe in data.metapath2vec_pos_enc_list:
                eval_pe = pe[eval_nodes].to(device)
                eval_data.metapath2vec_pos_enc_list.append(eval_pe)
        elif hasattr(data, 'metapath2vec_pos_enc') and data.metapath2vec_pos_enc is not None:
            # 处理单个PE（向后兼容）
        ...
        # 复制其他PE信息
        if hasattr(data, 'metapath_names'):
            eval_data.metapath_names = data.metapath_names
        if hasattr(data, 'metapath2vec_pe_paths'):
            eval_data.metapath2vec_pe_paths = data.metapath2vec_pe_paths
...
def train_model(model, data, train_mask, val_mask, test_mask, params, net_params, device):
    ...
    # 记录训练结束时的最终权重（用于JSON输出）
    final_head_weights = None
    if hasattr(model, 'use_head_weights') and model.use_head_weights:
        if hasattr(model, 'layers') and model.layers is not None:
            final_head_weights = []
            for i, layer in enumerate(model.layers):
                if hasattr(layer.attention, 'head_weights'):
                    weights = layer.attention.head_weights.detach().cpu().numpy().flatten()
                    final_head_weights.append({
                        'layer': i + 1,
    ...
    # 如果是MetapathGraphTransformerNet，显示头数信息
    if hasattr(model, 'layers') and model.layers is not None:
        print(f"\n模型架构信息:")
        print(f"  Transformer层数: {len(model.layers)}")
        if len(model.layers) > 0:
            first_layer = model.layers[0]
            if hasattr(first_layer, 'attention') and hasattr(first_layer.attention, 'num_heads'):
                print(f"  每层头数: {first_layer.attention.num_heads}")
                print(f"  头数计算: 1个节点特征头 + {first_layer.attention.num_heads - 1}个PE头")
    ...
    return {
        ...
        'final_head_weights': final_head_weights  # 添加最终权重信息
    }

def save_results(results, model, params, net_params, save_dir, results_df=None, data=None):
    """保存训练结果"""
    ...
    # 添加多元路径信息
    if data is not None and hasattr(data, 'metapath_names'):
        pe_summary['used_metapaths'] = data.metapath_names
        pe_summary['metapath2vec_pe_paths'] = data.metapath2vec_pe_paths if hasattr(data, 'metapath2vec_pe_paths') else []
    ...
    if hasattr(model, 'use_head_weights') and model.use_head_weights:
        head_weights_info = {
            'use_head_weights': True,
            'head_weights_by_layer': []
        }
        
        # 使用训练结束时的最终权重，而不是最佳模型的权重
        if results.get('final_head_weights') is not None:
            # 使用传递的最终权重信息
            for layer_info in results['final_head_weights']:
                weights = layer_info['weights']
                head_weights_info['head_weights_by_layer'].append({
        ...
        elif hasattr(model, 'layers') and model.layers is not None:
            # 备用方案：从当前模型获取权重
            for i, layer in enumerate(model.layers):
                if hasattr(layer.attention, 'head_weights'):
                    weights = layer.attention.head_weights.detach().cpu().numpy().flatten()
                    head_weights_info['head_weights_by_layer'].append({
                        'layer': i + 1,
                        'weights': [float(w) for w in weights],  # 保留完整精度
                        'weights_mean': float(np.mean(weights)),
                        'weights_std': float(np.std(weights)),
                        'weights_min': float(np.min(weights)),
                        'weights_max': float(np.max(weights))
        ...
        experiment_info['head_weights_info'] = head_weights_info
        print(f"头权重统计信息已添加到实验结果中")
    else:
        experiment_info['head_weights_info'] = {
            'use_head_weights': False
        }
