...
class WeightedNodeTypeAnalyzer:
    def __init__(self):
    ...
    def visualize_distribution(self, type_ratios, save_path):
        ...
        plt.axis('equal')
        plt.title('Node Type Distribution in Weighted RW Sampling')
    ...
    def visualize_bias(self, type_counts, save_path):
        ...
        plt.title('Node Type Bias Analysis in Weighted RW Sampling')
        plt.xticks(x, types)
        plt.legend()
    ...
    def run_analysis(self, edges, walk_length=10, num_walks=5):
        ...
        print("Building weighted heterogeneous graph...")
        # Build weighted heterogeneous graph
        G = nx.Graph()
        for edge_type, edge_matrix in enumerate(edges):
            indices = edge_matrix.nonzero()
            values = edge_matrix.data
            for (i, j), weight in zip(zip(indices[0], indices[1]), values):
        ...
        with open(os.path.join(self.result_dir, 'weighted_rw_stats.pkl'), 'wb') as f:
            pickle.dump(stats, f)
        
        # Visualize results
        self.visualize_distribution(type_ratios, 
                                  os.path.join(self.result_dir, 'distribution_weighted.png'))
        self.visualize_bias(type_counts, 
                           os.path.join(self.result_dir, 'view_bias_weighted.png'))
...
def main():
    ...
    data_path = os.path.join(base_dir, "metapath2vec_including_edge_weight", "process_data", "save_data", "Epoch12_edges.pkl")
    
    print(f"Reading weighted edge data from: {data_path}")
    with open(data_path, 'rb') as f:
        all_edges = pickle.load(f)
    
    # 只使用前4个边类型矩阵（对应原始数据集中的边类型）
    edges = all_edges[:4]
    print(f"Using first 4 edge types out of {len(all_edges)} total edge types")
    
    # Create analyzer and run analysis
    analyzer = WeightedNodeTypeAnalyzer()
