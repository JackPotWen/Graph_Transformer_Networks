...
def visualize_embeddings(paper_embeddings, author_embeddings, save_path=None, n_components=2):
    ...
    # 添加图例和标题
    plt.legend()
    plt.title('Paper-Author Embeddings Visualization (PCA) - ACM Dataset (Weighted)')
...
def main():
    # 设置命令行参数
    parser = argparse.ArgumentParser(description='使用PCA对ACM数据集的论文和作者节点嵌入进行可视化（带权重）')
    parser.add_argument('--vec_feature_path', type=str, 
                        default='/home/kuei-jan/github/Graph_Transformer_Networks/metapath2vec_including_edge_weight/weighted_metapath2vec/acm/vec_feature_pap_w200_l30_d128_c6_ns3.pkl',
                        help='节点嵌入文件路径')
    parser.add_argument('--save_path', type=str, 
                        default='/home/kuei-jan/github/Graph_Transformer_Networks/metapath2vec_including_edge_weight/weighted_metapath2vec/acm/paper_author_visualization_pca_weighted_PAP.png',
                        help='输出图片保存路径')
