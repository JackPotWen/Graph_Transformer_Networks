...
# 添加上级目录到Python路径以导入weighted_metapath2vec模块
...
def load_weighted_acm_data(data_path):
    """加载带权重的ACM数据集
    ...
    edge_weight_dict[('paper', 'belongs_to', 'subject')] = torch.from_numpy(
        ps_matrix.data.astype(np.float32))
    ...
    edge_weight_dict[('subject', 'contains', 'paper')] = torch.from_numpy(
        sp_matrix.data.astype(np.float32))
...
def train_weighted_metapath2vec(args):
    ...
    # 加载数据
    print("正在加载数据...")
    edge_index_dict, edge_weight_dict, num_nodes_dict = load_weighted_acm_data(args.data_path)
    ...
    with torch.no_grad():
        ...
        # 构建包含元路径和超参数信息的文件名
        filename = f'vec_feature_{metapath_abbr}_w{args.walks_per_node}_l{args.walk_length}_d{args.embedding_dim}_c{args.context_size}_ns{args.num_negative_samples}.pkl'
        save_path = os.path.join(args.save_dir, filename)
        with open(save_path, 'wb') as f:
            pickle.dump(node_emb.numpy(), f)
        print(f'节点嵌入已保存到: {save_path}')
    
    # 保存训练统计信息，同样包含元路径和超参数信息
    stats_filename = f'training_stats_{metapath_abbr}_w{args.walks_per_node}_l{args.walk_length}_d{args.embedding_dim}_c{args.context_size}_ns{args.num_negative_samples}.json'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, 
                        default='./metapath2vec_including_edge_weight/process_data/save_data/ACM_Epoch99_edges.pkl',
                        help='带权重的ACM数据集路径')
    parser.add_argument('--save_dir', type=str, 
                        default='/home/kuei-jan/github/Graph_Transformer_Networks/metapath2vec_including_edge_weight/weighted_metapath2vec/acm',
                        help='保存结果的目录')
    parser.add_argument('--embedding_dim', type=int, default=128,
                        help='嵌入维度')
    parser.add_argument('--walk_length', type=int, default=30,
                        help='随机游走长度')
    parser.add_argument('--context_size', type=int, default=6,
                        help='上下文窗口大小')
