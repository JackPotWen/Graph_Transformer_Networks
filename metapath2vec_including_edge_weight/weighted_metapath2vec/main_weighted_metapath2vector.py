...
from weighted_metapath2vec import WeightedMetaPath2Vec
from torch_geometric.utils import to_undirected
import json
from tqdm import tqdm
import time

def load_weighted_dblp_data(data_path):
    """加载带权重的DBLP数据集
    
    参数:
        data_path (str): edges.pkl文件路径
        
    返回:
        tuple: (edge_index_dict, edge_weight_dict, num_nodes_dict)
    """
    ...
    edge_index_dict[('author', 'writes', 'paper')] = torch.stack([
        torch.from_numpy(pa_edges[0].astype(np.int64)),  # 作者索引
        torch.from_numpy(pa_edges[1].astype(np.int64))   # 论文索引
    ])
    edge_weight_dict[('author', 'writes', 'paper')] = torch.from_numpy(
        pa_matrix.data.astype(np.float32))
    
    # 处理 AP 边 (论文-作者)
    ap_matrix = edges[1]  # CSC格式
    ap_edges = ap_matrix.nonzero()
    edge_index_dict[('paper', 'written_by', 'author')] = torch.stack([
        torch.from_numpy(ap_edges[0].astype(np.int64)),  # 论文索引
        torch.from_numpy(ap_edges[1].astype(np.int64))   # 作者索引
    ])
    edge_weight_dict[('paper', 'written_by', 'author')] = torch.from_numpy(
        ap_matrix.data.astype(np.float32))
    
    # 处理 PC 边 (论文-会议)
    pc_matrix = edges[2]  # CSR格式
    pc_edges = pc_matrix.nonzero()
    edge_index_dict[('paper', 'published_in', 'conference')] = torch.stack([
        torch.from_numpy(pc_edges[0].astype(np.int64)),  # 论文索引
        torch.from_numpy(pc_edges[1].astype(np.int64))   # 会议索引
    ])
    edge_weight_dict[('paper', 'published_in', 'conference')] = torch.from_numpy(
        pc_matrix.data.astype(np.float32))
    
    # 处理 CP 边 (会议-论文)
    cp_matrix = edges[3]  # CSC格式
    cp_edges = cp_matrix.nonzero()
    edge_index_dict[('conference', 'publishes', 'paper')] = torch.stack([
        torch.from_numpy(cp_edges[0].astype(np.int64)),  # 会议索引
        torch.from_numpy(cp_edges[1].astype(np.int64))   # 论文索引
    ])
    edge_weight_dict[('conference', 'publishes', 'paper')] = torch.from_numpy(
        cp_matrix.data.astype(np.float32))
    ...
    print(f"会议节点数量: {num_conferences}")
    for edge_type, edge_index in edge_index_dict.items():
        print(f"边类型 {edge_type}: {edge_index.shape[1]} 条边")
        print(f"权重范围: [{edge_weight_dict[edge_type].min():.4f}, {edge_weight_dict[edge_type].max():.4f}]")
    
    return edge_index_dict, edge_weight_dict, num_nodes_dict

def load_labels(label_path):
    """加载标签数据
    
    参数:
        label_path (str): labels.pkl文件路径
...
def evaluate_embeddings(embeddings, train_labels, val_labels, test_labels):
    ...
    """
    # 只使用作者节点的嵌入和标签
    author_embeddings = embeddings[:4057]  # 前4057个节点是作者
    
    # 准备训练数据
    X_train = author_embeddings[train_labels[:, 0]]
    y_train = train_labels[:, 1]
    
    # 准备验证数据
    X_val = author_embeddings[val_labels[:, 0]]
    y_val = val_labels[:, 1]
    
    # 准备测试数据
    X_test = author_embeddings[test_labels[:, 0]]
...
def train_weighted_metapath2vec(args):
    """训练带权重的MetaPath2Vec模型"""
    # 设置设备
    ...
    # 加载数据
    print("正在加载数据...")
    edge_index_dict, edge_weight_dict, num_nodes_dict = load_weighted_dblp_data(args.data_path)
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            # 获取作者节点嵌入
            author_emb = model('author').cpu()
            # 获取论文节点嵌入
            paper_emb = model('paper').cpu()
            # 获取会议节点嵌入
...
if __name__ == '__main__':
    ...
    # 训练模型
    train_weighted_metapath2vec(args) 