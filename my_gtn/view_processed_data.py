...
def load_metapath_data(data_path):
    """加载元路径数据"""
    with open(os.path.join(data_path, 'processed_metapath_edges.pkl'), 'rb') as f:
        metapath_edges = pickle.load(f)
    with open(os.path.join(data_path, 'processed_metapath_weights.pkl'), 'rb') as f:
...
def print_metapath_info(metapath_edges, metapath_weights):
    """打印每个元路径的基本信息"""
    print("="*50)
    print("处理后的DBLP元路径特征统计信息")
    print("="*50)
    
    print(f"\n总共保留了 {len(metapath_edges)} 个元路径")
    print("="*50)
    ...
    print("权重矩阵信息")
    print("="*50)
    for i, weight_matrix in enumerate(metapath_weights):
        print(f"\n权重矩阵 {i+1}:")
...
def main():
    ...
    data_path = os.path.join(os.path.dirname(current_dir), 'metapath_data', 'processed_DBLP')
